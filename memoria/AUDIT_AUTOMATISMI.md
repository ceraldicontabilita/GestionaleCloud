...
| Email/Documenti | **Fatture ESTERE via email → estrazione AI + fattura vera** (CORRETTO 2026-07-14: lo SDI/FatturaPA è solo italiano, i fornitori esteri mandano un PDF semplice, MAI un XML) | `services/email_monitor_service.py::sync_email_documents`, ramo dedicato `tipo == "fattura_estera_pdf"` → `routers/invoices/fatture_upload.py::process_fattura_estera_pdf` → `document_ai_extractor` + pipeline condivisa `import_parsed_invoice` (stessa di `process_xml_bytes`) | ✅ job `mittenti_email_sync`, ogni ora + pagina **Integrazioni → Mittenti Email** | ✅ | Con zero mittenti configurati il job è un no-op innocuo. **Aggiornato 14/07**: non più solo archiviazione — il PDF viene letto via AI e importato come fattura vera (fornitore auto-creato, prima nota provvisoria, partita aperta), quindi matching PayPal/bonifico e alert scadenza già esistenti la coprono. Se l'estrazione fallisce o non legge numero+importo, resta solo il PDF archiviato (nessun dato inventato). Limite noto: nessun punteggio di confidenza dall'estrazione AI (solo successo/fallimento) e nessuna estrazione della valuta (assunta EUR) — da monitorare sui primi casi reali. Fornitori con P.IVA in formato non standard (non 2 lettere + 11 cifre) non vengono agganciati/creati automaticamente (guardia `_piva_plausibile`, condivisa con le fatture italiane): la fattura viene comunque creata e riconciliata per importo/nome, solo l'anagrafica fornitore resta da collegare a mano |
