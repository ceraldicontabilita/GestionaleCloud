...
| Email/Documenti | **Fatture ESTERE via email → archiviazione PDF** (CORRETTO 2026-07-14: lo SDI/FatturaPA è solo italiano, i fornitori esteri mandano un PDF semplice, MAI un XML — la prima versione di questo fix instradava erroneamente al parser XML, che con un PDF non fa nulla) | `services/email_monitor_service.py::sync_email_documents`, ramo generico (tipo_documento="fattura_estera_pdf" → salvato in `documents_inbox`, stesso percorso già usato per cedolino/pagopa/ecc.) | ✅ RISOLTA: job `mittenti_email_sync`, ogni ora + pagina **Integrazioni → Mittenti Email** per aggiungere i mittenti man mano che arrivano (scelta utente: si scoprono solo dopo la prima fattura ricevuta) | ✅ | Con zero mittenti configurati il job è un no-op innocuo. **Limite noto**: il PDF viene solo archiviato in Documenti (categoria "Fatture estere (PDF)"), NON estratto/registrato in prima nota automaticamente — la funzione AI di estrazione esiste (`document_ai.py`/`save_fattura_to_gestionale`) ma scrive `invoices` con uno schema campi diverso da quello canonico (invoice_date/invoice_number/supplier_name) usato dal resto dell'app: collegarla richiede prima un'armonizzazione, decisione rimandata |
