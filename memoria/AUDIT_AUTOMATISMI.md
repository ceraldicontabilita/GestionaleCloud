...
| Email/Documenti | **Fatture ESTERE via email → estrazione AI + fattura vera + coda di verifica** (CORRETTO 2026-07-14: lo SDI/FatturaPA è solo italiano, i fornitori esteri mandano un PDF semplice, MAI un XML) | `services/email_monitor_service.py::sync_email_documents`, ramo dedicato `tipo == "fattura_estera_pdf"` → `routers/invoices/fatture_upload.py::process_fattura_estera_pdf` → `document_ai_extractor` + pipeline condivisa `import_parsed_invoice` (stessa di `process_xml_bytes`) → `routers/fatture_estera_verifica.py` (coda conferma/correzione + rating fornitore) | ✅ job `mittenti_email_sync`, ogni ora + pagina **Integrazioni → Mittenti Email** + pagina **Fatture estere da verificare** (link/alert) | ✅ | Con zero mittenti configurati il job è un no-op innocuo. Il PDF viene letto via AI e importato come fattura vera (fornitore auto-creato/agganciato anche su P.IVA estera non italiana, prima nota provvisoria, partita aperta), quindi matching PayPal/bonifico e alert scadenza già esistenti la coprono. Se l'estrazione fallisce o non legge numero+importo, resta solo il PDF archiviato (nessun dato inventato). **Aggiornato 14/07 (bis)**: ogni fattura importata entra in coda "da verificare" (alert `FAT_ESTERA_DA_VERIFICARE`, cliccabile) finché l'utente non conferma/corregge i dati — le correzioni alimentano un rating di affidabilità per fornitore, visibile sulla stessa pagina. Limite noto: nessun punteggio di confidenza dall'estrazione AI in sé (solo successo/fallimento — il rating nasce dalle verifiche umane, non da un self-score del modello) e nessuna estrazione della valuta (assunta EUR) — da monitorare sui primi casi reali. Se l'importo corretto riguarda una partita aperta già parzialmente pagata, non viene aggiornata in automatico (troppo rischioso), resta da sistemare a mano |
...
**🟠 Dormienti trovati (3)**:
1. ✅ **Fatture estere via email** — RISOLTA 2026-07-14: job `mittenti_email_sync` (ogni ora) + estrazione AI + fattura vera importata (aggancio automatico a matching PayPal/bonifico e alert scadenza).
2. **Anomalie IVA** — calcolate solo su richiesta, mai segnalate proattivamente. Ancora da decidere.
3. ~~**Libretti sanitari scadenze**~~ — non più applicabile: l'intero modulo libretti sanitari/contratti di lavoro è stato rimosso dal codice il 14/07/2026 (gestionale HR esterno, scelta utente).
