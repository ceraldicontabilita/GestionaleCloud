...
| 9 | Riconciliazione scadenzario | `fatture_module/ciclo_utils.py` (`cerca_match_bancario`/`esegui_riconciliazione`), usato da `scadenzario_fornitori.py` | **No** — `POST /api/scadenzario-fornitori/riconcilia-automatica`, zero chiamanti frontend (la pagina che usa questo prefix, GestioneCespiti.jsx, chiama solo le GET) | proprio matching per nome fornitore/importo | `scadenziario_fornitori`, `estratto_conto_movimenti`, `riconciliazioni` (un'altra collection ancora, diversa da `riconciliazioni_match`) |
