...
- **A7** — ✅ RISOLTA (2026-07-13, tre scelte utente). Registro UNICO = `movimenti_contabili` (motore §6.1). (1) TFR: accantonamento/liquidazione/ritenute/acconti in partita doppia via `registra_scrittura_semplice` (bilanciata, idempotente, senza saldi — il bilancio aggrega dalle sorgenti). (2) Ammortamenti: nuovo conto operativo `01.05.01 Fondo ammortamento` (→ ufficiale `41`), scrittura DARE 05.04.01 / AVERE 01.05.01. (3) Risultato d'esercizio: resta scrittura semplice con segno (niente epilogo, scelta utente) + guardia anti-doppia chiusura. (4) Libro giornale/mastro leggono `movimenti_contabili`; gli hook che scrivevano `scritture_contabili` a ogni fattura sono spenti (collection = archivio storico, `libro_giornale.py` = legacy). Test: tests/test_a7_scritture_semplici.py (7).
