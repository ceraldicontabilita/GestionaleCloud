...
path = Path('app/services/sumup_payout.py')
text = path.read_text()
old = '''        "conto_contabile": conti_pos.conto_accredito(GESTORE),
        "updated_at": now,
    }
'''
new = '''        "conto_contabile": conti_pos.conto_accredito(GESTORE),
...
'''
if old not in text:
    raise SystemExit('documento payout marker not found')
...
'''
if old not in text:
    raise SystemExit('comune payout marker not found')
text = text.replace(old, new, 1)
path.write_text(text)

contract = Path('tests/test_point10_pos_paypal_evidence_contract.py')
contract.write_text(r'''import asyncio
