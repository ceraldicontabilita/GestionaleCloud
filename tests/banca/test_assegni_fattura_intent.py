...
def test_assegno_compilato_prevale_su_fornitore_cassa_e_attende_estratto():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_intento_cassa"]
...
def test_estratto_conto_chiude_il_ciclo_e_imposta_flag_riconciliato():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_intento_ec"]
        await db.assegni.insert_one({
...
def test_non_collega_una_fattura_storica_di_un_altro_anno():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_intento_anno"]
        await db.assegni.insert_one({
...
def test_estratto_arriva_prima_della_fattura_e_xml_completa_tutto_senza_modale():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_ec_prima_xml"]
        await db.assegni.insert_one({
...
def test_numero_fattura_e_importo_univoci_collegano_anche_senza_beneficiario():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_numero_importo"]
        await db.assegni.insert_one({
            "id": "ass-solo-numero",
            "numero": "0208770650",
            "importo": 123.45,
...
def test_numero_fattura_e_importo_duplicati_restano_ambigui():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_numero_ambiguo"]
        await db.assegni.insert_one({
            "id": "ass-ambiguo",
            "numero": "0208770651",
            "importo": 90.00,
...
def test_modifica_anagrafica_non_degrada_un_assegno_incassato(monkeypatch):
    async def scenario():
        db = ClientArchivioMemoria()["assegno_incassato_protetto"]
        await db.assegni.insert_one({
            "id": "ass-incassato",
            "numero": "0208770652",
            "importo": 50.00,
...
def test_due_assegni_distinti_non_possono_saldare_due_volte_la_stessa_fattura():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_no_doppio_pagamento"]
        invoice = {
...
def test_due_rate_con_assegni_diversi_sono_ammesse_entro_totale():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_rate_valide"]
        invoice = {
...
def test_lista_segnala_sovra_attribuzione_storica_senza_modificare_dati(monkeypatch):
    async def scenario():
        db = ClientArchivioMemoria()["assegno_conflitto_storico"]
        await db.invoices.insert_one({
...
def test_endpoint_legacy_non_puo_sovrascrivere_una_fattura_gia_attribuita(monkeypatch):
    async def scenario():
        db = ClientArchivioMemoria()["assegno_endpoint_protetto"]
        await db.assegni.insert_one({
...
def test_collegamento_guidato_salva_numero_data_fornitore_e_relazione_bidirezionale(monkeypatch):
    async def scenario():
        db = ClientArchivioMemoria()["assegno_collegamento_guidato"]
        await db.assegni.insert_one({
...
def test_riprocessamento_storico_collega_assegno_incassato_alla_fattura_univoca():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_riprocessamento_storico"]
        await db.assegni.insert_one({
...
def test_riprocessamento_non_indovina_tra_due_fatture_identiche():
    async def scenario():
        db = ClientArchivioMemoria()["assegno_riprocessamento_ambiguo"]
        await db.assegni.insert_one({
