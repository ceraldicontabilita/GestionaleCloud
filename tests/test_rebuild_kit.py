...
def test_rebuild_kit_is_complete_integral_and_single_root(tmp_path: Path):
    ...
    with zipfile.ZipFile(output) as archive:
        ...
        assert len([name for name in names if name.startswith(f"{prefix}03_PAGINE/") and Path(name).name[:2].isdigit() and name.endswith(".md")]) == 76
        logic_names = [name for name in names if name.startswith(f"{prefix}03_PAGINE/LOGICA_JSON/") and name.endswith(".json")]
        assert len(logic_names) == 76
    ...
    assert manifest["counts"]["pages"] == 76
    assert manifest["counts"]["page_logic_contracts"] == 76
