...
@pytest.fixture
def db(monkeypatch):
    finto = ClientArchivioMemoria()["dedup_causali_test"]
...
def test_la_bonifica_marca_solo_l_eccedenza_cross_file():
    db = ClientArchivioMemoria()["bonifica_test"]
...
def test_un_gruppo_con_una_sola_forma_non_viene_toccato():
    ...
    db = ClientArchivioMemoria()["bonifica_test2"]
    _semina_storico(
        db,
...
def test_la_riga_gia_lavorata_e_quella_che_resta():
    ...
    db = ClientArchivioMemoria()["bonifica_test3"]
    _semina_storico(
        db,
        _ec("a1", SDD_CORTO, "ElencoEntrateUscite.csv"),
...
def test_l_analisi_non_scrive_niente():
    db = ClientArchivioMemoria()["bonifica_test4"]
    _semina_storico(
        db,
        _ec("a1", SDD_CORTO, "ElencoEntrateUscite.csv"),
        _ec("b1", SDD_LUNGO, "ESTRATTO 2026.csv"),
