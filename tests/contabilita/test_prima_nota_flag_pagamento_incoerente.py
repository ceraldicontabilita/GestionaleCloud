...
def test_paid_alias_e_id_laterale_sono_rilevati_e_ripristinati(monkeypatch):
    async def scenario():
        db = ClientArchivioMemoria()["test_prima_nota_flag_incoerente"]
        monkeypatch.setattr(
...
def test_fattura_paid_con_movimento_attivo_non_viene_toccata(monkeypatch):
    async def scenario():
        db = ClientArchivioMemoria()["test_prima_nota_flag_coerente"]
        monkeypatch.setattr(
            manutenzione.Database, "get_db", staticmethod(lambda: db)
        )
        await db["invoices"].insert_one({
...
def test_provvisoria_include_banca_senza_estratto_e_link_cancellato():
    async def scenario():
        db = ClientArchivioMemoria()["test_provvisoria_evidenza"]
        fatture = [
...
def test_pagamento_parziale_mantiene_aperto_solo_il_residuo():
    async def scenario():
        db = ClientArchivioMemoria()["test_provvisoria_residuo"]
        fattura = {
            "id": "fattura-parziale",
            "total_amount": 100.0,
...
def test_parcella_con_ritenuta_chiude_sul_netto_fornitore():
    async def scenario():
        db = ClientArchivioMemoria()["test_parcella_netto"]
        fattura = {
            "id": "parcella-1",
...
def test_parcella_con_ritenuta_mostra_solo_il_netto_residuo():
    async def scenario():
        db = ClientArchivioMemoria()["test_parcella_residuo"]
        fattura = {
            "id": "parcella-2",
            "total_amount": 3806.40,
            "ritenuta_importo": 600.0,
