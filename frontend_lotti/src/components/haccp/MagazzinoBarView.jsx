...
function MessageBox({ title, text, onRetry }) {
  return (
    <div style={{ textAlign: "center", color: "#9aa593", padding: "48px 18px" }}>
      <div style={{ fontSize: 22, fontWeight: 900, color: "#e6e0d4", marginBottom: 10 }}>{title}</div>
      <div style={{ fontSize: 14, lineHeight: 1.45, marginBottom: 18 }}>{text}</div>
      {onRetry ? (
        <button onClick={onRetry} style={{ border: "1px solid #6b7669", background: "#3f5a4e", color: "#fff", borderRadius: 14, padding: "11px 18px", fontWeight: 900 }}>
          Riprova
        </button>
      ) : null}
    </div>
...
function KeypadPopup({ titolo, value, onChange, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 4000, background: "rgba(0,0,0,.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#3f5a4e", borderRadius: 22, padding: 18, width: "100%", maxWidth: 320, boxShadow: "0 20px 60px rgba(0,0,0,.5)" }}>
        <div style={{ color: "#9aa593", fontSize: 13, fontWeight: 800, marginBottom: 6, textAlign: "center" }}>{titolo}</div>
        <div style={{ background: "#2a3329", color: "#fff", borderRadius: 14, padding: "14px", fontSize: 30, fontWeight: 900, textAlign: "center", marginBottom: 12, minHeight: 40 }}>
          {String(value) || "0"}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
          {["1","2","3","4","5","6","7","8","9",",","0","⌫"].map(t => (
            <button key={t} onClick={() => premi(t)}
              style={{ border: "none", borderRadius: 14, padding: "16px 0", fontSize: 22, fontWeight: 900, background: "#56442d", color: "#fff", cursor: "pointer" }}>
              {t}
            </button>
          ))}
        </div>
...
function ProductCard({ p, onReload, authorizeAction }) {
  ...
  return (
    <div style={{ background: "#fff", color: "#2a3329", borderRadius: 18, padding: 14, border: "1px solid #e6e0d4" }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        ...
        <div style={{ textAlign: "right", minWidth: 72 }}>
          ...
          <div style={{ fontSize: 11, color: "#6b7669" }}>{unita}</div>
          {p.pezzi_per_collo > 1 && p.colli != null && (
            <div style={{ fontSize: 10, color: "#9aa593", fontWeight: 700 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        ...
        <button onClick={() => setKeypad(true)}
          style={{ width: 80, border: "1px solid #c7cfc2", background: "#fff", borderRadius: 12, padding: "10px", fontWeight: 900, textAlign: "center", fontSize: 16, cursor: "pointer" }}>
          {qty || "0"}
        </button>
...
export default function MagazzinoBarView({ onBack, soloLavagna = false }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#2a3329", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ padding: 16 }}>
        {loading ? (
          ...
          <>
            ...
            <div style={{ background: "#3f5a4e", borderRadius: 18, padding: 14, marginBottom: 16 }}>
              ...
              {reqProd ? (
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <button onClick={() => setReqQta(q => Math.max(1, (Number(q) || 1) - 1))} style={{ width: 42, height: 42, borderRadius: 12, border: "none", background: "#56442d", color: "#fff", fontSize: 22, fontWeight: 900 }}>−</button>
                    <div style={{ width: 36, textAlign: "center", fontSize: 20, fontWeight: 900 }}>{reqQta}</div>
                    <button onClick={() => setReqQta(q => (Number(q) || 1) + 1)} style={{ width: 42, height: 42, borderRadius: 12, border: "none", background: "#56442d", color: "#fff", fontSize: 22, fontWeight: 900 }}>+</button>
                  </div>
                  <button onClick={() => setReqUnita(u => u === "collo" ? "pezzo" : "collo")} style={{ border: "1px solid #6b7669", background: "transparent", color: "#e6e0d4", borderRadius: 12, padding: "10px 12px", fontWeight: 900 }}>{reqUnita === "collo" ? "📦 Cartoni" : "🔢 Pezzi"}</button>
                  ...
                  <button onClick={() => { setReqProd(null); setReqSearch(""); }} style={{ border: "none", background: "transparent", color: "#9aa593", fontWeight: 900 }}>✕</button>
                </div>
              ) : (
                <>
                  <input value={reqSearch} onChange={(e) => setReqSearch(e.target.value)} placeholder="Cerca il prodotto… (es. prosecco)" style={{ width: "100%", boxSizing: "border-box", padding: "13px 14px", borderRadius: 14, border: "1px solid #56442d", background: "#2a3329", color: "#fff", fontSize: 16 }} />
                  {reqMatches.map(p => (
                    <button key={p.id} onClick={() => setReqProd(p)} style={{ display: "block", width: "100%", textAlign: "left", marginTop: 8, padding: "12px 14px", borderRadius: 12, border: "1px solid #56442d", background: "#0b1220", color: "#fff", fontWeight: 800, fontSize: 15 }}>
                      {p.nome} <span style={{ color: "#6b7669", fontWeight: 700 }}>· stock {p.stock ?? 0}</span>
                    </button>
                  ))}
                </>
              )}
            ...
            <div style={{ fontWeight: 900, fontSize: 16, marginBottom: 10 }}>📺 Da prelevare ({richieste.length}) <span style={{ color: "#6b7669", fontSize: 12, fontWeight: 800 }}>si aggiorna da sola</span></div>
            {richieste.length === 0 ? (
              <MessageBox title="Nessuna richiesta" text="Quando il bar chiede un rifornimento, compare qui in automatico." />
            ) : richieste.map(r => (
              <div key={r.id} style={{ background: "#3f5a4e", borderRadius: 18, padding: 14, marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: "#9aa593", fontSize: 12, fontWeight: 800, marginTop: 2 }}>chiesto da {r.richiesto_da || "bar"} · {String(r.created_at || "").slice(11, 16)}</div>
                </div>
                <button onClick={() => annullaRichiesta(r)} style={{ border: "none", background: "transparent", color: "#6b7669", fontWeight: 900, fontSize: 18 }}>✕</button>
          ...
          <>
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cerca prodotto o fornitore" style={{ width: "100%", boxSizing: "border-box", padding: "13px 14px", borderRadius: 16, border: "1px solid #56442d", background: "#3f5a4e", color: "#fff", marginBottom: 12, fontSize: 16 }} />
            <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto", alignItems: "center" }}>
              {[["tutti", "Tutti"], ["bar", "Bar"], ["fornitori", "Fatture"]].map(([id, label]) => (
                <button key={id} onClick={() => setSource(id)} style={{ padding: "8px 16px", borderRadius: 999, border: "1px solid #56442d", background: source === id ? "#c59a5f" : "#3f5a4e", color: source === id ? "#fff" : "#9aa593", fontWeight: 900 }}>{label}</button>
              ))}
              {/* Giacenze per anno di fatturazione (solo lotti da fattura) */}
              <select value={anno} onChange={(e) => setAnno(e.target.value)}
                style={{ marginLeft: "auto", padding: "8px 12px", borderRadius: 999, border: "1px solid #56442d", background: anno ? "#c59a5f" : "#3f5a4e", color: anno ? "#fff" : "#9aa593", fontWeight: 900, flexShrink: 0 }}>
