      ...
      {open && (
        <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(15,23,42,0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 14, width: 340, boxShadow: "0 22px 70px rgba(15,23,42,0.35)", overflow: "hidden" }}>
            <div style={{ padding: "14px 16px", borderBottom: "1px solid #e6e0d4", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#3f5a4e" }}>Report HACCP Mensile</div>
              <button onClick={() => setOpen(false)} style={{ border: 0, background: "transparent", color: "#6b7669", cursor: "pointer", padding: 4 }}><X size={18} /></button>
            </div>

            <div style={{ padding: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 110px", gap: 10, marginBottom: 16 }}>
                <select value={mese} onChange={e => setMese(Number(e.target.value))} style={{ padding: "9px 11px", borderRadius: 8, border: "1px solid #e6e0d4", fontSize: 14, background: "#fff" }}>
                  {MESI.map((m, i) => <option key={i + 1} value={i + 1}>{m}</option>)}
                </select>
                <select value={anno} onChange={e => setAnno(Number(e.target.value))} style={{ padding: "9px 11px", borderRadius: 8, border: "1px solid #e6e0d4", fontSize: 14, background: "#fff" }}>
                  {anni.map(a => <option key={a} value={a}>{a}</option>)}
                </select>
              </div>

              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setOpen(false)} style={{ flex: 1, padding: "10px 0", borderRadius: 9, border: "1px solid #e6e0d4", background: "#faf7f0", fontWeight: 700, color: "#6b7669", cursor: "pointer" }}>Annulla</button>
