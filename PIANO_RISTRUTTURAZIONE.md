...
|---|---|---|---|
| 2026-09-24 | RST-GC-13-17-18-PULIZIA-RICONCILIAZIONE | 🟢 | [PR #713](https://github.com/ceraldicontabilita/GestionaleCloud/pull/713), merge `362616255ea75a817d2ac60313e1c555ab4f9177` (26 commit di `fix/audit-gc` applicati con `git am`, albero identico al bundle). CI verde (backend, frontend ERP, build HR/Lotti/Menu, E2E, audit layout); `/api/health` e `/lotti/api/health` confermano il merge esatto su gestionalecloud.onrender.com e impresasemplice.online, `hydration_errors: 0`. Letture HACCP senza sessione rifiutate (401), registrazione su giorno futuro rifiutata (422); la prova a vista delle schede sul tablet richiede il PIN e resta al titolare. GC-13: tolti 41 import e le variabili inutilizzate, `export_entries_pdf` senza chiamanti rimosso; ruff F pulito su `app/`. Il primo giro aveva tolto anche `mesi_it` da `extract_period_from_text`, che lo usava: lo ha preso la suite completa (`tests/documenti/test_email_full_download_mittenti_attendibili.py`), ripristinato. B904: i 243 `raise` dentro `except` conservano la causa (`from exc`), trasformazione meccanica sulle posizioni di ruff, nessun nome ombreggiato. GC-18: la riconciliazione PagoPA/AdER/TARI (`riconciliazione_completa.py`) legava il primo movimento generico a ogni documento; ora abbina solo per IUV o numero, oppure per importo esatto con un movimento unico non gia' usato. Timbro di tracciabilita' dell'etichetta lotto con lo stato vero. GC-17 (decisione del titolare 24/09, tutte si'): cedolino PDF abbinato solo al suo dipendente (prima un cedolino qualsiasi del mese); F24 da file solo con mese, anno e tipo tributo coerenti e un candidato unico (prima bastava l'anno o niente); PartenoPay rifiuta file senza sha256 e indicizza con l'hash calcolato; tolte `extract_supplier_name` (doppione mai usato: il fornitore e' gia' nel punteggio e nel filtro duro) e `_clean_zucchetti_s` (dannosa: ogni s fra minuscole diventava spazio). 9 test nuovi, rossi sul codice precedente. Suite backend 4.783 verde, ERP 362, Lotti 104, build di ERP, HR, Lotti e Menu ok (24/09) |
| 2026-09-24 | RST-GC-CONGELATORI-MENU-PULIZIA | 🟢 | [PR #713](https://github.com/ceraldicontabilita/GestionaleCloud/pull/713), merge `362616255ea75a817d2ac60313e1c555ab4f9177` (26 commit di `fix/audit-gc` applicati con `git am`, albero identico al bundle). CI verde (backend, frontend ERP, build HR/Lotti/Menu, E2E, audit layout); `/api/health` e `/lotti/api/health` confermano il merge esatto su gestionalecloud.onrender.com e impresasemplice.online, `hydration_errors: 0`. Letture HACCP senza sessione rifiutate (401), registrazione su giorno futuro rifiutata (422); la prova a vista delle schede sul tablet richiede il PIN e resta al titolare. Congelatori registrabili dal tablet con azione correttiva (prima nessuna pagina poteva scrivere una temperatura di congelatore); giorni chiusi di nuovo visibili nelle schede (il frontend cercava `data_formattata`, l'API restituisce `data`); admin Menu su database vero (prima `mockData.js` e salvataggio in console); tolti 66 file frontend mai importati e 38 pacchetti Python mai importati (~229 MB), strumenti di sviluppo in `backend/requirements-dev.txt`. Suite 4.768 verde in ambiente pulito, ERP 362, Lotti 104. Conteggio dati HACCP in sola lettura (24/09): 384 temperature 2026 con nome ma senza firma, 0 fuori soglia; 31.056 valori 2022-2026 tutti dentro soglia, 41 valori distinti, documenti creati il 29/08/2026; sanificazioni 2022-2026 intestate all'operatrice designata, apparecchi con ~10% «non eseguita» e prodotto vuoto. Nessuna modifica ai dati: serve decisione del titolare |
| 2026-09-24 | RST-GC-HACCP-FATTI-VERI | 🟢 | [PR #713](https://github.com/ceraldicontabilita/GestionaleCloud/pull/713), merge `362616255ea75a817d2ac60313e1c555ab4f9177` (26 commit di `fix/audit-gc` applicati con `git am`, albero identico al bundle). CI verde (backend, frontend ERP, build HR/Lotti/Menu, E2E, audit layout); `/api/health` e `/lotti/api/health` confermano il merge esatto su gestionalecloud.onrender.com e impresasemplice.online, `hydration_errors: 0`. Letture HACCP senza sessione rifiutate (401), registrazione su giorno futuro rifiutata (422); la prova a vista delle schede sul tablet richiede il PIN e resta al titolare. Registri HACCP solo con fatti firmati: via i fermi frigo e i «non usato» casuali del calendario chiusure; sanificazione senza operatore designato e prodotto scritti di default; turno 07:00 che non dichiara piu' «conforme» (lo fa il responsabile dopo il giro, firmato: `POST /haccp-auto/dichiara-conformi-oggi`); firma da PIN o sessione tablet; niente giorni futuri; correzioni in `sostituisce`; stampe coi firmatari veri; report mensile che perdeva le sanificazioni apparecchi. Modulo unico `servizi/registro_haccp.py`. Suite backend 4.765 verde, Lotti frontend 104 verde. Dati gia' in archivio (384 rilevazioni 2026 sorteggiate, schede intestate all'operatrice designata) non toccati: servono conteggio e autorizzazione |
| 2026-09-24 | RST-GC-SICUREZZA-BUILD | 🟢 | [PR #713](https://github.com/ceraldicontabilita/GestionaleCloud/pull/713), merge `362616255ea75a817d2ac60313e1c555ab4f9177` (26 commit di `fix/audit-gc` applicati con `git am`, albero identico al bundle). CI verde (backend, frontend ERP, build HR/Lotti/Menu, E2E, audit layout); `/api/health` e `/lotti/api/health` confermano il merge esatto su gestionalecloud.onrender.com e impresasemplice.online, `hydration_errors: 0`. Letture HACCP senza sessione rifiutate (401), registrazione su giorno futuro rifiutata (422); la prova a vista delle schede sul tablet richiede il PIN e resta al titolare. regex prodotti finiti (backspace al posto di `\b`); login admin HR con blocco tentativi e cookie `Secure` sotto `/hr`; backup Menu fuori dal loop, archivio verificato prima di svuotare le tabelle; tolti 3 endpoint admin Menu che rispondevano success senza salvare; default mutabili; axios 1.20 in Lotti e Menu; CI su Python 3.12.10 e build di HR/Lotti/Menu. Aperto: build Render con `yarn.lock` (serve la dashboard), repo pubblico su GitHub |
| 2026-09-24 | RST-GC-02H-HACCP-NON-ATTENDIBILI | 🟡 | [PR #714](https://github.com/ceraldicontabilita/GestionaleCloud/pull/714). Strada A decisa dal titolare: le registrazioni HACCP in archivio senza firma verificata si segnano (`non_attendibili` nel documento), i valori restano intatti. Regola unica in `servizi/haccp_attendibilita.py` e `frontend_lotti/src/utils/attendibilita.js`; `/haccp-attendibilita/anteprima` e `/segna` (admin, `dry_run` per difetto, `conferma=SEGNA`); report mensile, stampe e tablet mostrano «n.a.» fuori da conteggi e percentuali. Conteggio SQL in sola lettura (24/09): 30.768 + 384 + 1.104 + 2.088 + 9.368 + 1.467 = 45.179 da segnare; intatti 1.536 chiusi, 1.224 non rilevati, 96 caselle aperte, 80 N/D, 2 valori annidati riportati come anomalie. Suite backend 4.797 verde, Lotti 108 test e build. Scrittura sui dati: solo dopo il deploy, con anteprima mostrata al titolare e suo ok |
| 2026-09-24 | RST-ORDINI-MIGLIOR-FORNITORE | 🟡 | Ordini → Confronto riscritto sull'idea dell'app «Ceraldi Ordini» (Netlify): articolo = marca + prodotto + formato + confezione, miglior fornitore per articolo. Fonte: righe delle fatture XML attive del gestionale (non il ponte Lotti fermo, che escludeva Siro e Sud Ingrosso, `escluso` per la tracciabilita' HACCP). Motore deterministico `servizi/confronto_fornitori.py`: formato letto dal testo (CL.50 = 50CL = ML500, LT.1,5 = CL.150, KG.3X6, CTX24; lotti e gradi ignorati), prezzo per pezzo (cartone se l'unita' e' un cartone o la quantita' e' minore della confezione), fornitore = P.IVA, accorpamento automatico solo se certo, coppie probabili decise da una persona (`/confronto-fornitori/decisione`), gruppo con prezzi per pezzo oltre 3x = «da verificare», nessun migliore. Tolti `/food-cost/confronto-prodotto` e `/food-cost/confronto-prezzi` (doppioni sul dizionario). Prova sulle 896 fatture vere: 55 articoli con piu' fornitori (il listino ne aveva 2), 52 con migliore, 3 da verificare, 35 proposte. Vista smartphone verificata a 390 px senza scroll orizzontale. Suite 4.838 verde, Lotti 111 test e build |
...
| 2026-09-23 | RST-AV3-01B-ERRORI-MIDDLEWARE | 🟢 | [PR #691](https://github.com/ceraldicontabilita/GestionaleCloud/pull/691), merge `cb57db0d14745d21b23c2d07008d8dbcfbfb564d`, servito su entrambi i domini; live: 401 con `code` NON_AUTENTICATO e `message` italiano. Il middleware di autenticazione rispondeva a mano `{detail}` in 16 punti, fuori dal contratto (verificato live dopo #688). `errore_http` unico in `error_handler.py`: stesso `detail`, `code`, `message` italiano, `correlation_id`, intestazioni conservate. Suite backend 4.696 verde. |
| 2026-09-23 | RST-0508BG-SCHEDA-RICETTA-UNICA | 🟢 | PR #691, merge `cb57db0`: bundle Lotti live con «Apri scheda» e messaggio di riavvio verificati. Segnalazione del titolare (screenshot 23/09 09:45–09:47): sulle card di reparto serviva «Apri scheda» per vedere il modo di preparazione, e nella scheda compariva `<!DOCTYPE html>…`. Cause: due schede ricetta per la stessa funzione (`SchedaRicettaKiosk` nascondeva il procedimento se mancante, 199 ricette su 328) e `apiError` che mostrava tale e quale la pagina HTML del proxy Render durante il riavvio del deploy. Una sola scheda (`SchedaRicettaChiaraModal`, carica anche per id, allergeni, modifica rapida del tablet), `SchedaRicettaKiosk` eliminata, bottone «Apri scheda» da 44px sulle card, `apiError` riconosce le pagine HTML, «Riprova» nel calcolo dose. 101 test Lotti e build verdi, bundle senza colori vietati |
| 2026-09-23 | RST-0508BH-PROCEDIMENTI-WEB | 🟡 | Richiesta del titolare: dove manca il procedimento (199 ricette su 328, tutte operative e con ingredienti) inserirlo da ricerca web. Provenienza nel dato: `procedimento_origine` (`manuale`/`web`), `procedimento_fonte` (url, titolo, differenze con i nostri ingredienti), `procedimento_da_verificare`. Import unico `POST /ricette/procedimenti-web` (admin, `dry_run` per difetto) che scrive solo dove il procedimento manca: seconda esecuzione a zero. Una modifica a mano rende il procedimento del titolare; conferma esplicita `POST /ricette/{id}/procedimento/conferma`. La scheda mostra fonte, differenze e «Confermo il procedimento». Regole della ricerca: fonti italiane tracciate, testo riscritto, nessuna dose, tempi e temperature solo se della fonte, fonte incompatibile = non trovato col motivo |
| 2026-09-23 | RST-UX-02-TESTATA-PAGINA | 🟡 | Passo 2 del documento di design: `PageHeader` (l'unica testata ERP, anche via `PageLayout`) riceve la famiglia della pagina e le pastiglie (etichetta, valore, nota, tono ok/attenzione/male/neutro, azione da 44px). Applicata **solo** a Prima nota: i quattro riquadri Riporto/Entrate/Uscite/Saldo diventano le pastiglie della testata (stessi numeri, stesso calcolo; rosso solo sul saldo negativo), in Provvisori le due code di fatture aperte. Su telefono le schede Cassa/Banca vanno a capo invece di tagliarsi. Da propagare alle altre pagine dopo che il titolare l'ha vista |
| 2026-09-23 | RST-UX-01-COLONNA-NAVIGAZIONE | 🟢 | [PR #695](https://github.com/ceraldicontabilita/GestionaleCloud/pull/695), merge `1ada8e0`, servito in produzione (bundle con la colonna, senza «Altro» né «Vai a sezione»). Documento di design del titolare (23/09), Parte IV passo 1: la barra con «Altro» (11 sezioni su 16 nascoste) diventa una colonna sempre visibile, 9 gruppi per momento del lavoro, mappa unica `NAV_GRUPPI` in `navigation.config.js` (colonna, menù mobile, 404). La tendina «Vai a sezione» di `HubTabs` sparisce: le schede degli hub sono tutte visibili; Contabilità tiene solo «Indietro», le sue 15 sezioni sono nella colonna. Fondo pagina alzato per il bottone dell'assistente. Nessun calcolo toccato. Catalogo pagine: aggiunte «Regole banca» e «Posizione auto e driver», mancanti (test rosso già su main). Passi successivi dello stesso documento: testata di pagina, nomi al posto delle sigle, regola del colore, pagine pesanti |
...
| 2026-09-25 | RST-00A9-RATE-ASSEGNI-XML | 🟢 | [PR #724](https://github.com/ceraldicontabilita/GestionaleCloud/pull/724), merge `6ea558d45d3258ae36c87cb31d4bc6af1f4fbb39`: le proposte per assegni confrontano le rate MP02 della fattura XML, anziché soltanto il totale fattura; conservano numero/scadenza/importo di ciascuna rata e distinguono MP05. La conferma manuale accetta lo scarto documentato di un centesimo sull'ultima rata senza azzerarlo. Fattura Di Massa n. 20: rate di 3.000/3.000/3.000/3.000,01 euro; quattro assegni da 3.000 euro lasciano 0,01 euro non pagato. Le due fatture A 2000 Costruzioni da 9.760 euro dichiarano MP05, quindi gli assegni omonimi restano da confermare per prova, non sono collegati automaticamente. 41 test backend e 20 frontend mirati verdi, CI pre-merge `36097585538` e CI main `36097921363` verdi. `/lotti/api/health` su `impresasemplice.online` ha restituito esattamente il merge `6ea558d45d3258ae36c87cb31d4bc6af1f4fbb39` il 25/09/2026. Workflow Produzione main `36097921374` ancora in attesa al momento della registrazione: non viene dichiarato verde. Nessuna associazione contabile reale creata dalla PR; rollback tramite revert. |
| 2026-09-25 | RST-00A9-PAYPAL-BANCA-IMPORT | 🟢 | [PR #727](https://github.com/ceraldicontabilita/GestionaleCloud/pull/727), merge `bb74218daab76345967c99dc71b4960842b94d56`: quando l'import di un estratto bancario **ufficiale** produce righe con causale PayPal, la sincronizzazione PayPal Reporting acquisisce i mesi interessati prima del motore canonico banca/PayPal/fatture. Il documento resta bancario; accrediti, addebiti e fornitori non sono equiparati dalla sola parola o dal solo importo. Risposta dell'import con stato della sincronizzazione, inclusi credenziali assenti/errori; il resto della riconciliazione continua. 56 test PayPal locali verdi; CI pre-merge `36099090631` ed E2E isolato `36099090539` verdi. `/lotti/api/health` su `impresasemplice.online` ha restituito **esattamente** il merge `bb74218daab76345967c99dc71b4960842b94d56` il 25/09/2026; workflow Produzione main `36099378429` ancora in corso al momento della registrazione, non dichiarato verde. Nessun PDF reale caricato né associazione contabile reale creata dalla PR. Prossima micro-tranche: il PDF singola transazione `Transazioni - PayPal.pdf` contiene un ID del 29/04/2025 ma il parser MSR attuale restituisce 0 righe; il report `paypal transazioni 2025-2026 (1).PDF` contiene 76 righe e termina il 04/08/2025, quindi non prova i pagamenti 2026. |
| 2026-09-25 | RST-00A9-PAYPAL-PDF-REALi | 🟢 | [PR #731](https://github.com/ceraldicontabilita/GestionaleCloud/pull/731), merge `38f46a16fedb40d6c90d0084ad6ecc530c6e6ac1`: il parser canonico legge il PDF di singola transazione con ID, data operativa e importo con segno, e la cronologia annuale italiana con `ID/Codice`, stato e importi; eliminata la falsa lettura precedente di 76 righe senza ID e a 0 euro. Due dettagli dello stesso giorno senza conto dichiarato mantengono identità distinte. Prova read-only sui file reali: 6 MSR mensili gennaio-giugno 2025 = 68 ID; cronologia = 76 ID, dei quali 68 già presenti e 8 nuovi; dettaglio = 1 ID già presente. Import in archivio di test: 76 transazioni uniche, nessuna copia per sovrapposizione. 60 test PayPal locali, CI pre-merge `36100497515`, E2E isolato `36100497523` e CI main `36100800172` verdi. `/lotti/api/health` ha restituito **esattamente** `38f46a16fedb40d6c90d0084ad6ecc530c6e6ac1` il 25/09/2026; workflow Produzione main `36100800171` ancora in corso al momento della registrazione, non dichiarato verde. Nessun PDF reale caricato né associazione contabile reale applicata. Resta aperto l'aggancio immediato quando il PDF PayPal arriva da Documenti > Import, anziché dalla pagina PayPal. |
| 2026-09-25 | RST-00A9-PAYPAL-INGRESSI-UNICI | 🟢 | [PR #733](https://github.com/ceraldicontabilita/GestionaleCloud/pull/733), merge `e2284b52a5121c75c993b48b068e3c2768e0e9bd`: una sola pipeline PayPal collega in ordine fattura, banca, fattura da Documenti > Import, pagina PayPal, lotto Drive, API e ripasso periodico; eliminati i blocchi triplicati nei chiamanti vivi. La risposta dell'import espone i collegamenti e le ambiguità; la sincronizzazione API include per intero l'ultimo giorno richiesto e applica la riconciliazione. Nessuna associazione nasce dalla sola dicitura PayPal o dal solo importo. 85 test mirati locali e 17 test Drive/documenti verdi; CI pre-merge `36102227375`, E2E isolato `36102227354` verdi. `/lotti/api/health` ha restituito **esattamente** il merge `e2284b52a5121c75c993b48b068e3c2768e0e9bd` il 25/09/2026; workflow Produzione main `36102536060` ancora in corso al momento della registrazione, non dichiarato verde. Nessun PDF reale caricato né movimento reale associato dalla PR; la prova persistente va eseguita separatamente dopo l'import autorizzato. |
| 2026-09-25 | RST-0508BA-IMPORT-FOTO-RICETTE | 🟡 | [PR #739](https://github.com/ceraldicontabilita/GestionaleCloud/pull/739), merge `ee996bdaa234840831268dcdcbb5e30982158544`: importatore amministrativo per mappa con ID e nome esatti e cartella PNG. Usa l'upload canonico Supabase Storage, hash SHA-256, backup precedente e sincronizzazione Menu; non ricrea ricette eliminate, non usa fuzzy matching e salta foto già canoniche. 117 test frontend e build locali verdi; CI/E2E pre-merge `36158919569`/`36158919562` e CI main `36159315775` verdi. `/lotti/api/health` su `impresasemplice.online` ha confermato esattamente il merge; il workflow Produzione main `36159315773` era ancora in corso alla registrazione. Mappa Drive verificata: 368 PNG/ID; 305 candidati operativi invariati, 41 non più operativi, 20 già su Storage, 2 nomi cambiati da verificare. **Nessuna nuova foto è stata associata:** Chrome ha impedito la selezione locale dei file; attesa l'abilitazione esplicita dell'accesso file per l'estensione o selezione manuale. Non toccare Render. |
