...
- Ultima tranche conclusa: [PR #647](https://github.com/ceraldicontabilita/GestionaleCloud/pull/647), eliminazione del collegamento automatico delle foto alle ricette per nome.
- Codice pubblicato e verificato: **`42c1e596591092e2cbfc76adf27c525f32fdef44`**.
- Prova di produzione: [workflow 35687061739](https://github.com/ceraldicontabilita/GestionaleCloud/actions/runs/35687061739), E2E isolato, audit layout e smoke verdi; `/lotti/api/health` sui due domini ha restituito il commit di merge esatto; [CI 35687061752](https://github.com/ceraldicontabilita/GestionaleCloud/actions/runs/35687061752) verde. Le foto già salvate non sono state eliminate o riassegnate.
...
| 2026-09-22 | RST-0508AH-PANINI-SEED | 🟡 | In lavorazione: eliminare `seed_panini_rosticceria` da startup e i suoi helper senza chiamanti. Oggi l'upsert per nome a ogni avvio può ricreare un panino eliminato e sovrascrivere le modifiche di un record operativo; la correzione non deve cancellare dati già persistiti. Test comportamentale di due riavvii, PR/CI/E2E/merge/health da verificare. Il seed storico delle ricette solo-nome resta tranche distinta. |
...
**Rilascio verificato:** RST-0508AE della PR #647 è pubblicato con `42c1e596591092e2cbfc76adf27c525f32fdef44`, confermato da `/lotti/api/health`. La correzione puntuale delle due schede HR amministratore/dipendente in produzione è stata verificata con login tablet e `dipendente_id` distinti; i PIN non sono conservati nel piano. **Non sono concluse** la qualificazione completa della Prima Nota, la gestione PIN centralizzata dalla UI, l'identità unica con mapping HR/Lotti per tutto lo storico, l'unificazione dati/outbox né la fusione HR/Lotti/Menu/frontend. Nessuna di queste attività va dichiarata completata per effetto della sola bonifica.
