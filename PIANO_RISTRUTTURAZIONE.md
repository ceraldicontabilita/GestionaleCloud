...
| 2026-09-22 | RST-0508AN-FOTO-CESTINO-DRIVE | 🟡 | In corso migrazione riprendibile delle foto ancora richiamate dalle ricette eliminate: backup integrale del cestino prima del primo upload, raggruppamento per `foto_id` legacy, upload una sola volta per immagine e aggiornamento immediato di tutte le voci collegate. La clonazione foto tra base e variante usa ora lettura e nuova copia Drive; il percorso `foto_files` resta solo come fallback temporaneo fino a riferimenti=0. 26 test mirati locali verdi; PR, CI/E2E, esecuzione sui 72 file distinti, prova di ripristino e health da completare. |
