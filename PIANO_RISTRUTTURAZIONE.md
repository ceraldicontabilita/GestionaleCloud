...
| 2026-09-21 | RST-0508M-STATI-ANOMALIE-HACCP | 🟡 | Audit web reale: Anomalie mostra 1 aperta su 17, Registro HACCP mostra erroneamente 17 aperte. Il riepilogo cerca `risolta` minuscolo mentre il dominio salva `Risolta`/`Chiusa`. Unificare gli stati del dominio e riallineare il conteggio; test, CI e produzione da verificare |
...
**Rilascio verificato:** RST-0508L della PR #628 è pubblicato e accessibile live con `a9264987`; le PR #603-#627 e le precedenti PR #599-#602 restano pubblicate. La correzione puntuale delle due schede HR amministratore/dipendente in produzione è stata verificata con login tablet e `dipendente_id` distinti; i PIN non sono conservati nel piano. **Non sono concluse** la qualificazione completa della Prima Nota, la gestione PIN centralizzata dalla UI, l'identità unica con mapping HR/Lotti per tutto lo storico, l'unificazione dati/outbox né la fusione HR/Lotti/Menu/frontend. Nessuna di queste attività va dichiarata completata per effetto della sola bonifica.
