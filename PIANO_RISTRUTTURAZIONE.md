...
| 2026-09-22 | RST-0508R-ANNULLAMENTO-LOTTI | 🟡 | Audit web Tracciabilità: «Elimina» invocava il DELETE che cancellava fisicamente un lotto e interrompeva la cronologia HACCP. Nuovi percorsi annulla/ripristina motivati con ID dipendente verificato, record conservato in archivio e movimenti idempotenti; UI e chiamante migrati, DELETE e vecchio endpoint senza chiamanti `elimina-senza-tracciabilita` rimossi. I lotti già inviati al banco o usati come componenti richiedono rettifica del flusso collegato. 45 test backend del flusso e build Lotti locali verdi; CI/rilascio da verificare |
...
**Rilascio verificato:** RST-0508Q della PR #633 è pubblicato e accessibile live con `ef9aa47c`; le PR #603-#632 e le precedenti PR #599-#602 restano pubblicate. La correzione puntuale delle due schede HR amministratore/dipendente in produzione è stata verificata con login tablet e `dipendente_id` distinti; i PIN non sono conservati nel piano. **Non sono concluse** la qualificazione completa della Prima Nota, la gestione PIN centralizzata dalla UI, l'identità unica con mapping HR/Lotti per tutto lo storico, l'unificazione dati/outbox né la fusione HR/Lotti/Menu/frontend. Nessuna di queste attività va dichiarata completata per effetto della sola bonifica.
