...
| 2026-09-22 | RST-0508U-EMETTITORE-PRODUZIONE-BANCO | 🟡 | Audit del router: POST `/produzioni/` senza chiamanti dell'app creava registrazione e vendita banco separate, senza lotto né ID dipendente; il flusso vivo usa `/registra-produzione-lotto` e il servizio vendita canonico. Rimuovere l'emettitore duplicato e aggiornare la documentazione; test, CI e rilascio da verificare |
...
**Rilascio verificato:** RST-0508T della PR #636 è pubblicato e accessibile live con `96332e0c`; le PR #603-#635 e le precedenti PR #599-#602 restano pubblicate. La correzione puntuale delle due schede HR amministratore/dipendente in produzione è stata verificata con login tablet e `dipendente_id` distinti; i PIN non sono conservati nel piano. **Non sono concluse** la qualificazione completa della Prima Nota, la gestione PIN centralizzata dalla UI, l'identità unica con mapping HR/Lotti per tutto lo storico, l'unificazione dati/outbox né la fusione HR/Lotti/Menu/frontend. Nessuna di queste attività va dichiarata completata per effetto della sola bonifica.
