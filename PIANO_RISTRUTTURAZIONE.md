...
| 2026-09-22 | RST-0508S-PIPELINE-LOTTI-STORICI | 🟡 | Audit della pipeline notturna: lo step lotti cancellava fisicamente gli esauriti oltre un anno e poteva sovrascrivere uno stato di richiamo con `scaduto`. Riscrivere lo step con il calcolo canonico della scadenza, preservare storia, annullamenti e richiami. Test locale verde; CI/rilascio da verificare |
...
**Rilascio verificato:** RST-0508R della PR #634 è pubblicato e accessibile live con `91f3ce7a`; le PR #603-#633 e le precedenti PR #599-#602 restano pubblicate. La correzione puntuale delle due schede HR amministratore/dipendente in produzione è stata verificata con login tablet e `dipendente_id` distinti; i PIN non sono conservati nel piano. **Non sono concluse** la qualificazione completa della Prima Nota, la gestione PIN centralizzata dalla UI, l'identità unica con mapping HR/Lotti per tutto lo storico, l'unificazione dati/outbox né la fusione HR/Lotti/Menu/frontend. Nessuna di queste attività va dichiarata completata per effetto della sola bonifica.
