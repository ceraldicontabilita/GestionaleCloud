...
async def create_prodotto(prodotto: ProdottoVendita):
    ...
    if prodotto.ricetta_id:
        ...
        if ricetta:
            ...
            try:
                from app.lotti.routers.food_cost import calcola_food_cost_ricetta
                ...
                prodotto.costo_produzione = costo.get("costo_porzione") or costo.get(
                    "costo_totale", 0
                )
            except Exception as e:
                logger.warning(f"[prodotti_vendita] Food cost per ricetta fallito: {e}")
    ...
    if prodotto.prezzo_vendita > 0 and prodotto.costo_produzione > 0:
        ...
        prodotto.margine_percentuale = round(
            (prodotto.margine_euro / prodotto.prezzo_vendita) * 100, 1
        )
...
async def aggiorna_prodotto(prodotto_id: str, prodotto: ProdottoVendita):
    ...
    if not existing:
        existing = await db.prodotti_vendita.find_one(
            {"id": {"$regex": f"^{prodotto_id[:12]}"}}, {"_id": 0}
        )
        if existing:
            prodotto_id = existing["id"]
    if not existing:
        # Crea nuovo se non esiste
        import uuid

        prodotto_id = str(uuid.uuid4())
        existing = {"id": prodotto_id}

    pv = float(prodotto.prezzo_vendita or 0)
    cp = float(prodotto.costo_produzione or 0)
    iva = float(prodotto.iva if prodotto.iva is not None else -1)
    iva_aliquota = float(prodotto.iva_compresa_aliquota or 10)

    update_data = prodotto.model_dump()
    update_data["updated_at"] = datetime.now(timezone.utc).isoformat()

    # Calcolo IVA: se iva=-1 → prezzo è già IVA compresa
    if iva == -1 and pv > 0:
        update_data["prezzo_ivato"] = round(pv * (1 + iva_aliquota / 100), 2)
        update_data["prezzo_netto"] = round(pv / (1 + iva_aliquota / 100), 2)
    elif iva > 0 and pv > 0:
        update_data["prezzo_ivato"] = round(pv * (1 + iva / 100), 2)
        update_data["prezzo_netto"] = pv
    else:
        update_data["prezzo_ivato"] = pv
        update_data["prezzo_netto"] = pv
...
async def aggiorna_prezzo_prodotto(prodotto_id: str, prezzo_vendita: float, iva: float = 10):
    ...
    iva_val = float(prodotto.get("iva", -1) or -1)
    iva_aliquota = float(prodotto.get("iva_compresa_aliquota", 10) or 10)

    # Calcolo IVA compresa
    if iva_val == -1:
        prezzo_ivato = round(prezzo_vendita * (1 + iva_aliquota / 100), 2)
        prezzo_netto = round(prezzo_vendita / (1 + iva_aliquota / 100), 2)
    elif iva_val > 0:
        prezzo_ivato = round(prezzo_vendita * (1 + iva_val / 100), 2)
        prezzo_netto = prezzo_vendita
    else:
        prezzo_ivato = prezzo_vendita
        prezzo_netto = prezzo_vendita
...
async def get_presenza_prodotto(prodotto_id: str):
    ...
    ricette_con_prod = []
    async for r in db.ricette.find(
        {"ingredienti.nome": {"$regex": nome[:15], "$options": "i"}}, {"_id": 0, "nome": 1, "id": 1}
    ):
        ricette_con_prod.append(r.get("nome", r.get("id", "?")))

    # Cerca tra i lotti fornitori (materie prime reali in stock)
    mag = await db.lotti_fornitori.find_one(
        {
            "$or": [
                {"prodotto_nome": {"$regex": nome[:15], "$options": "i"}},
                {"prodotto_nome_norm": {"$regex": nome[:15], "$options": "i"}},
            ]
        },
        {"_id": 0, "prodotto_nome": 1, "fornitore": 1, "quantita_disponibile": 1},
    )
...
async def delete_prodotto_cascade(
 ...
 _admin=Depends(require_admin)):
    ...
    if not prod:
        # Ricerca parziale ID
        prod = await db.prodotti_vendita.find_one(
            {"id": {"$regex": f"^{prodotto_id[:12]}"}}, {"_id": 0}
        )
        if prod:
            prodotto_id = prod["id"]
    if not prod:
        # Prodotto non trovato — elimina comunque senza errore
        return {"success": True, "eliminati": {"nota": "Prodotto già eliminato o non presente"}}
    ...
    if da_magazzino:
        # magazzino non è una collection separata: i lotti fornitori sono la fonte
        # non si eliminano lotti fornitore in cascade (dati tracciabilità HACCP)
        results["magazzino"] = 0
...
async def delete_prodotto(prodotto_id: str):
    ...
    if result.deleted_count == 0:
        ...
        prod = await db.prodotti_vendita.find_one({"id": {"$regex": f"^{prodotto_id[:12]}"}})
        if prod:
            await db.prodotti_vendita.delete_one({"_id": prod["_id"]})
    return {"success": True}
...
async def sync_prodotti_acquaviva():
    ...
    prodotti_acq = await db.dizionario_prodotti.find(
        {"fornitore": {"$regex": "acquaviva", "$options": "i"}}, {"_id": 0}
    ).to_list(1000)

    esistenti = await db.prodotti_vendita.find(
        {"fonte": "acquaviva"}, {"_id": 0, "nome": 1, "id": 1}
    ).to_list(1000)
    ...
    for p in prodotti_acq:
        ...
        if nome_key in nomi_esistenti:
            await db.prodotti_vendita.update_one({"id": nomi_esistenti[nome_key]}, {"$set": data})
            updated += 1
        else:
...
async def ricalcola_costi_acquaviva():
    ...
    prodotti = await db.prodotti_vendita.find(
        {"fonte": "acquaviva", "pezzi_cartone": {"$gt": 0}}, {"_id": 0}
    ).to_list(1000)
...
async def sync_prodotti_da_ricette():
    ...
    prodotti_esistenti = await db.prodotti_vendita.find(
        {}, {"_id": 0, "ricetta_id": 1, "id": 1, "nome": 1}
    ).to_list(1000)
    ricette_con_prodotto = {
        p.get("ricetta_id"): p.get("id") for p in prodotti_esistenti if p.get("ricetta_id")
    }
    ...
    from app.lotti.routers.food_cost import trova_prodotto_dizionario, converti_in_kg
    ...
    for ricetta in ricette:
        ...
        costo = (
            round(costo_totale / porzioni, 4) if porzioni > 0 and costo_totale > 0 else costo_totale
        )
...
async def auto_categorie():
    ...
    RULES = [
        ...
        (
            ...
            "Pasticceria > Torte e Monoporzioni",
        ),
        # Creme e preparazioni base
        (
            ["crema", "panna", "pasta choux", "ricotta zuccherata", "pan di spagna", "biscotto"],
            "Pasticceria > Basi e Creme",
        ),
        # Pizze e focacce
...
async def get_statistiche_margini():
    ...
    prodotti = await db.prodotti_vendita.find({"attivo": True}, {"_id": 0}).to_list(1000)
    ...
    margini = [
        float(p.get("margine_percentuale", 0) or 0)
        for p in prodotti_con_prezzo
        if float(p.get("margine_percentuale", 0) or 0) > 0
    ]
