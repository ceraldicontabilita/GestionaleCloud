...
async def get_temp_cottura_oggi():
    ...
    docs = await db.temperature_cottura.find({"data": oggi}, {"_id": 0}).sort("ora", 1).to_list(200)
    return docs
...
async def get_storico_cottura(giorni: int = 30):
    ...
    docs = (
        await db.temperature_cottura.find({"data": {"$gte": data_limite}}, {"_id": 0})
        .sort("data", -1)
        .to_list(1000)
    )
    return docs
...
async def aggiorna_temperatura_cottura(cottura_id: str, payload: dict = Body(...)):
    result = await db.temperature_cottura.update_one(
        {"id": cottura_id}, {"$set": {k: v for k, v in payload.items() if k not in ("id", "_id")}}
    )
...
async def statistiche_cottura(giorni: int = 30):
    ...
    data_limite = (datetime.now(timezone.utc) - timedelta(days=giorni)).strftime("%Y-%m-%d")
    docs = await db.temperature_cottura.find({"data": {"$gte": data_limite}}, {"_id": 0}).to_list(
        1000
    )
