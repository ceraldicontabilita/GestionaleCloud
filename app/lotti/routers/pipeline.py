    ...
    for r in ricette:
        ...
        for ing in r.get("ingredienti") or []:
            if isinstance(ing, str):
                nomi.add(ing.lower().strip())
        allergeni = _allergeni_da_ingredienti(nomi)
        await db.ricette.update_one(
            {"id": r["id"]}, {"$set": {"allergeni": allergeni, "allergeni_auto": True}}
        )
        aggiornate += 1
...
async def step_prezzi_dizionario(log: dict):
    # data_fattura è in formati misti (dd/mm/yyyy dall'import, ISO storico):
    # un $gte come stringa ISO filtrava un sottoinsieme quasi casuale.
    # Il filtro "ultimi 45 giorni" va fatto su date vere, in Python.
    from app.lotti.routers.utils import parse_data_flessibile
    da = (datetime.now(timezone.utc) - timedelta(days=45)).date()
    tutte = await db.fatture.find(
        {}, {"_id": 0, "prodotti": 1, "fornitore": 1, "data_fattura": 1}
    ).to_list(10000)
    fatture = [f for f in tutte
               if (parse_data_flessibile(f.get("data_fattura")) or date.min) >= da]
    aggiornati = 0
    for f in fatture:
            ...
            res = await db.dizionario_prodotti.update_one(
                {"nome_normalizzato": nome_norm},
                {
                    "$set": {
                        ...
                        # nome campo allineato al resto del backend (era il typo
                        # gemello "ultima_data_fattura", che nessuno leggeva →
                        # il refresh notturno della data d'acquisto era perso).
                        "ultima_fattura_data": f.get("data_fattura"),
...
async def step_costi_acquaviva(log: dict):
    from app.lotti.routers.utils import parse_data_flessibile
    # "ultime 2 fatture" per data VERA: il sort Mongo su data_fattura (formato
    # misto dd/mm/yyyy + ISO) era lessicografico → i costi Acquaviva potevano
    # essere aggiornati da fatture vecchie.
    _tutte = (
    ...
    _tutte.sort(key=lambda f: parse_data_flessibile(f.get("data_fattura")) or date.min, reverse=True)
    fatture = _tutte[:2]
    aggiornati = 0
    for f in fatture:
        for p in f.get("prodotti", []):
            ...
            if not kg_cart or not peso_g or peso_g <= 0:
                continue
...
async def get_stato():
    ...
    ricette_no_all = await db.ricette.count_documents(
        {"$or": [{"allergeni": {"$exists": False}}, {"allergeni": []}, {"allergeni": None}]}
    )
    prod_no_prezzo = await db.dizionario_prodotti.count_documents(
