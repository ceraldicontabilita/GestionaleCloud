...
async def get_controllo_olio_oggi():
    ...
    docs = await db.controllo_olio.find({"data": oggi}, {"_id": 0}).sort("ora", 1).to_list(100)
    return docs
...
async def get_storico_olio(giorni: int = 30):
    ...
    docs = (
        await db.controllo_olio.find({"data": {"$gte": data_limite}}, {"_id": 0})
        .sort("data", -1)
        .to_list(500)
    )
    return docs
...
async def aggiorna_controllo_olio(controllo_id: str, payload: dict = Body(...)):
    ...
    result = await db.controllo_olio.update_one(
        {"id": controllo_id}, {"$set": {k: v for k, v in payload.items() if k not in ("id", "_id")}}
    )
...
async def statistiche_olio(giorni: int = 30):
    ...
    data_limite = (datetime.now(timezone.utc) - timedelta(days=giorni)).strftime("%Y-%m-%d")
    docs = await db.controllo_olio.find({"data": {"$gte": data_limite}}, {"_id": 0}).to_list(1000)
