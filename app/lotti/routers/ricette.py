...
async def leggi_foto(foto_id: str):
    """Serve l'immagine persistita su Supabase Storage. URL con ?v=<versione>: contenuto di una
...
async def auto_assegna_reparti(applica: bool = True, _admin=Depends(require_admin)):
    """Riallinea ricette operative e riferimenti dei fornitori al reparto certo.

    Le schede dei fornitori restano nascoste sul tablet finché non sono
    attivate. Il reparto è corretto anche nel ricettario consultabile.
    I nomi ambigui conservano il reparto corrente. applica=false è dry-run.
    """
    ricette = await db.ricette.find(
        {},
        {
