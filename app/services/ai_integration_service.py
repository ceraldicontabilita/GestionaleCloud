    ...
    try:
        ...
        try:
            if detected_type == "fattura" and parsed.get("fornitore", {}).get("partita_iva"):
                ...
                # Il netto letto da un modello non e' una prova: un cedolino
                # si registra solo dal suo PDF col motore unico. Qui lo si
                # consegna alla coda che quel motore legge (`category`
                # busta_paga di documents_inbox, giro della posta).
                await db[collection].update_one(
                    {"id": document_id},
                    {"$set": {"category": "busta_paga", "processed": False}},
                )
                result["saved_to"] = collection
                result["requires_cedolino_engine"] = True
