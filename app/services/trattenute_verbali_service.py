...
# Stati in cui si aspetta il riscontro nel cedolino
...
async def verifica_trattenute_cedolino(event: Dict[str, Any], db) -> Dict[str, Any]:
    ...
    for t in trattenute:
        ...
        # Claim atomico: aggiorna solo se ancora in attesa (idempotente)
        res = await db[COLL_TRATTENUTE].update_one(
            {"id": tid, "stato": {"$in": list(STATI_ATTESA_VERIFICA)}},
            {"$set": update},
        )
        ...
        await log_evento(
            ...
            fonte="cedolino_import",
            dettaglio=(
                f"Cedolino {periodo} {t.get('dipendente') or t.get('dipendente_nome', '')}: "
                + (
                    f"trattenuta verbale {t.get('numero_verbale', '')} trovata "
                    f"(voce '{parola_trovata}')"
                    if parola_trovata
                    else f"trattenuta verbale {t.get('numero_verbale', '')} NON trovata"
                )
            ),
        ...
        if parola_trovata:
            ...
            if t.get("verbale_id"):
                try:
                    ...
                    logger.exception("Errore chiusura verbale %s", t.get("verbale_id"))
            ...
            await genera_alert(
                ...
                COLL_TRATTENUTE,
                (
                    f"Trattenuta verbale {t.get('numero_verbale', '')} "
                    f"({t.get('dipendente') or t.get('dipendente_nome', '')}, "
                    f"€{float(t.get('importo_da_recuperare') or t.get('importo') or 0):.2f}) "
                    f"attesa nel cedolino {periodo} ma non trovata"
                ),
                db,
                extra={"cedolino_id": cedolino_id, "verbale_id": t.get("verbale_id")},
            )
