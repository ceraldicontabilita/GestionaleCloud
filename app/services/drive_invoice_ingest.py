...
La radice configurata puo' contenere il lifecycle direttamente oppure, come
nel Drive reale, fascicoli annidati per anno:
    FATTURE/<anno>/DA ELABORARE
Solo le cartelle lifecycle canoniche entro due livelli vengono scansionate.
Elaborate ed Errori restano sorelle dell'inbox che ha originato il documento.
"""
import asyncio
import gc
import io
...
def start_background_rebuild(db, *, reset: bool = False) -> bool:
    ...
    if _rebuild_lock.locked() or _sync_lock.locked():
        return False
    _rebuild_task = asyncio.create_task(ricostruisci_archivio_drive_lotto(db, reset=reset))
    return True
...
def is_configured() -> bool:
    return bool(
        ...
        and _folder_id()
        and (
            settings.GOOGLE_SERVICE_ACCOUNT_JSON_FATTURE
...
def _build_drive_service():
    ...
    if creds is None:
        logger.error("Drive ingest: %s", err)
        return None
    try:
        from googleapiclient.discovery import build
        return build("drive", "v3", credentials=creds, cache_discovery=False)
    except Exception as e:
        logger.error("Drive ingest: errore costruzione service: %s", e)
        return None
...
def _source_folders(service, parent_id: str) -> List[tuple[str, str]]:
    """Cartelle da rileggere per rebuild, incluse quelle annidate per anno."""
    folders: List[tuple[str, str]] = [("radice", parent_id)]
    for item in discover_lifecycle_folders(service, parent_id, max_depth=2):
        folders.append((item["relative_path"], item["folder_id"]))
    return folders
...
async def _do_sync(db) -> Dict[str, Any]:
    ...
    try:
        ...
        for context in contexts:
            ...
            for file_info in _list_xml_files(service, source_id):
                queued.append({
                    **file_info,
        ...
        for f in batch:
            ...
            try:
                ...
                logger.error("Drive ingest: errore su %s: %s", fname, e)
                result["errors"] += 1
                result["details"].append({
                    "file": fname,
                    "source_path": f["_source_path"],
        ...
        logger.error("Drive ingest: errore sync: %s", e)
...
async def ricostruisci_archivio_drive_lotto(
    ...
    *,
    ...
    async with _rebuild_lock:
        ...
        cursor = str(previous.get("cursor")) if can_resume and previous.get("cursor") else None
        started_at = (
        ...
        try:
            ...
            for file_info in batch:
                ...
                try:
                    content = await asyncio.to_thread(_download_bytes, service, file_info["id"])
        ...
        try:
            ...
            for index, file_info in enumerate(unique_files.values(), start=1):
                try:
                    ...
                    logger.exception("Ricostruzione Drive: errore sul file %s", file_info["name"])
                    result["errors"] += 1
                    if len(result["details"]) < 20:
                        result["details"].append({
                            "file": file_info["name"], "error": str(exc)[:200],
...
async def verifica_quadratura_elaborate(db) -> Dict[str, Any]:
    """Verifica tutte le cartelle Elaborate canoniche, anche quelle per anno."""
    if not is_configured():
        return {
            "status": "not_configured",
    ...
    try:
        ...
        for folder in elaborate_folders:
            files = _list_xml_files(service, folder["folder_id"])
                ...
                try:
                    ...
                    if st == "duplicate":
                        ...
                        if len(esito["dettaglio_recuperati"]) < 50:
                            esito["dettaglio_recuperati"].append(
                                f"{folder['relative_path']}/{fname}"
                            )
                    else:
                        esito["errori"] += 1
                        if len(esito["dettaglio_errori"]) < 20:
                            esito["dettaglio_errori"].append({
                                "file": f"{folder['relative_path']}/{fname}",
        ...
        logger.error("Quadratura Elaborate: errore: %s", e)
        return {"status": "error", "message": str(e)}
    ...
    await db[_SYNC_STATE_COLLECTION].update_one(
        ...
        {"$set": {
            ...
            "last_quadratura_result": {k: esito[k] for k in (
                "totale_file_elaborate", "quadrati", "recuperati", "errori", "cartelle_elaborate"
            )},
        }},
        upsert=True,
    )
