        ...
        if cedolino_data.get("cessato") and dipendente_id:
            try:
                ...
                if not gia_cessato:
                    ...
                    from app.hr.services.event_bus import propagate_event as _pe, EventTypes as _et
...
async def processa_tutti_cedolini_pdf(db, pdf_data: str, filename: str) -> Dict[str, Any]:
    ...
    if not cedolini:
        try:
            from app.hr.parsers.payslip_parser_v2 import parse_payslip_pdf
    ...
    for ced in cedolini:
        ...
        if ced_pdf_text and not ced.get("cessato"):
            try:
                from app.hr.parsers.busta_paga_multi_template import detect_cessazione
        ...
        try:
            from app.hr.services.salari_unificati_v2 import processa_cedolino_v2
