      ...
      <div style={{ padding: 40 }}>
        ...
        >
          ...
          <div style={{ fontSize: 13, color: '#64748b', marginTop: 8 }}>
            Verbale "{verbaleId}" non trovato nel database. Potrebbe essere stato eliminato o il
            numero potrebbe non essere corretto.
          </div>
          <button
            onClick={() => navigate(-1)}
            style={{
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 18, fontWeight: 'bold' }}>
                {formatEuro(verbale?.importo || 0)}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 11, opacity: 0.7 }}>DRIVER</div>
              <div style={{ fontSize: 14 }}>
                {verbale?.driver || verbale?.veicolo_info?.driver || '-'}
              </div>
            </div>
            <div>
                      ...
                      onClick={async () => {
                        try {
                          const res = await api.get(
                            `/api/verbali-noleggio/pdf/${verbale.numero_verbale}?indice=${pdf.indice}`
                          );
              ...
              {verbale?.stato === 'pagato' || verbale?.stato_pagamento === 'pagato' ? (
                ...
                >
                  ...
                  >
                    {verbale?.importo ? (
                      <>
                        <span style={{ color: '#666' }}>Importo</span>
                        <span style={{ fontWeight: 600 }}>{formatEuro(verbale.importo)}</span>
                      </>
                    ) : null}
                    {verbale?.data_pagamento ? (
                      <>
                        <span style={{ color: '#666' }}>Data</span>
                        <span>{formatDateIT(verbale.data_pagamento)}</span>
                      </>
                    ) : null}
                    {verbale?.psp ? (
                      <>
                        <span style={{ color: '#666' }}>PSP</span>
                        <span>{verbale.psp}</span>
                      </>
                    ) : null}
                    {verbale?.metodo_pagamento ? (
                      <>
                        <span style={{ color: '#666' }}>Metodo</span>
                        <span>{verbale.metodo_pagamento}</span>
                      </>
                    ) : null}
                    {verbale?.fonte_riconciliazione ? (
                      <>
                        <span style={{ color: '#666' }}>Fonte</span>
                        <span>{verbale.fonte_riconciliazione}</span>
                      </>
                    ) : null}
                    {verbale?.iuv ? (
                      <>
                        <span style={{ color: '#666' }}>IUV</span>
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const res = await api.post(
                          `/api/verbali-noleggio/${encodeURIComponent(vid)}/cerca-pagamento`
                        );
                        const d = res.data;
                        if (d.trovato) {
                          alert(
              ...
              {verbale?.movimento_info && (
                <div style={{ marginTop: 16, fontSize: 13 }}>
                  ...
                  >
                    ...
                    <div>
                      Causale:{' '}
                      {verbale.movimento_info.causale || verbale.movimento_info.descrizione}
                    </div>
                  </div>
                </div>
              )}
              ...
              >
                ...
                <div style={{ fontSize: 13, lineHeight: 2 }}>
                  ...
                  >
                    ...
                    <strong style={{ fontFamily: 'monospace' }}>
                      {verbale.veicolo_info.contratto || '-'}
                    </strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: '#6b7280' }}>Periodo:</span>
                    <strong>
                      {formatDateIT(verbale.veicolo_info.data_inizio)} -{' '}
                      {formatDateIT(verbale.veicolo_info.data_fine)}
                    </strong>
                  </div>
                </div>
              </div>
