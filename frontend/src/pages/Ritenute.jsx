...
export default function Ritenute() {
  ...
  const badge = stato => {
    const s = STATI[stato] || { label: stato || '—', bg: '#f2f0e9', fg: '#4c4a44' };
    return (
  ...
  return (
    <div style={{ padding: '14px clamp(10px, 3vw, 28px)', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {error ? (
        ...
        <div style={{ background: 'white', border: '1px solid #e6e3d9', borderRadius: 12, overflowX: 'auto' }}>
            ...
            <thead>
              <tr style={{ background: '#f6f4ee', borderBottom: '2px solid #e6e3d9' }}>
                {['Fornitore', 'Fattura', 'Ritenuta', 'Importo', 'Scadenza (16 mese succ.)', 'Pagata il', 'F24', 'Stato'].map((h, i) => (
                  <th key={h} style={{ padding: '9px 10px', fontSize: 11, color: '#7a776e', textTransform: 'uppercase', textAlign: i === 3 ? 'right' : 'left' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
